"""Скачивание публичных артефактов AiZynthFinder (USPTO).

Тянет с Zenodo / figshare ONNX-модели политик (expansion, filter, ringbreaker),
сжатые шаблоны реакций и ZINC stock (HDF5), затем пишет ``config.yml`` с
абсолютными путями к файлам — готовый минимальный конфиг для запуска планировщика.

По умолчанию каталог: ``data/aizynthfinder`` (относительно текущей рабочей директории).
Запуск из корня репозитория: ``uv run python scripts/download_public_data.py``
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="download_public_data",
        description="Скачать модели, шаблоны и stock AiZynthFinder и сгенерировать config.yml.",
    )
    parser.add_argument(
        "path",
        nargs="?",
        default="data/aizynthfinder",
        help="каталог для файлов (по умолчанию: data/aizynthfinder)",
    )
